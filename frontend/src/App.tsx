...
export default function App() {
  ...
  return <div className="site-shell">
    ...
    <main className="page-content">
      ...
      {view === 'projects' && <ProjectsView projects={projects} selected={selectedProject} setSelected={setSelectedProject} data={projectData} onMilestoneCreated={(milestone) => setProjectData((data) => ({ ...data, milestones: [...data.milestones, milestone] }))} onMilestoneUpdated={(milestone) => setProjectData((data) => ({ ...data, milestones: data.milestones.map((item) => item.id === milestone.id ? milestone : item) }))} />}
...
function CitizenView({ problems, query, setQuery, onSubmitted }: { problems: Problem[]; query: string; setQuery: (v: string) => void; onSubmitted: (p: Problem) => void }) {
  const [form, setForm] = useState({ title: '', description: '', category: '', submitter_type: 'individual', district: 'Ranchi', latitude: '23.3441', longitude: '85.3096' });
  const [photo, setPhoto] = useState<File | null>(null); const [busy, setBusy] = useState(false); const [error, setError] = useState('');
  const update = (key: string, value: string) => setForm((f) => ({ ...f, [key]: value }));
  const submit = async (event: FormEvent) => { event.preventDefault(); setError(''); setBusy(true); try { const data = new FormData(); Object.entries(form).forEach(([k, v]) => { if (k !== 'category' || v) data.append(k, String(v)); }); if (photo) data.append('photo', photo); onSubmitted(await api.submitProblem(data)); setForm({ ...form, title: '', description: '', category: '' }); setPhoto(null); } catch (e) { setError(e instanceof Error ? e.message : 'Unable to register problem.'); } finally { setBusy(false); } };
...
function ProjectsView({ projects, selected, setSelected, data, onMilestoneCreated, onMilestoneUpdated }: { projects: Project[]; selected: Project | null; setSelected: (p: Project | null) => void; data: { milestones: Milestone[]; outcome: Outcome | null; partnerships: Partnership[]; messages: Message[] }; onMilestoneCreated: (milestone: Milestone) => void; onMilestoneUpdated: (milestone: Milestone) => void }) {
  const [milestoneTitle, setMilestoneTitle] = useState(''); const [milestoneDescription, setMilestoneDescription] = useState(''); const [milestoneDueDate, setMilestoneDueDate] = useState(''); const [milestoneError, setMilestoneError] = useState('');
  const addMilestone = async (event: FormEvent) => { event.preventDefault(); if (!selected) return; setMilestoneError(''); try { const milestone = await api.createMilestone(selected.id, { title: milestoneTitle, description: milestoneDescription || null, due_date: milestoneDueDate || null }); onMilestoneCreated(milestone); setMilestoneTitle(''); setMilestoneDescription(''); setMilestoneDueDate(''); } catch (e) { setMilestoneError(e instanceof Error ? e.message : 'Unable to create milestone.'); } };
  const updateMilestone = async (milestone: Milestone, status: 'pending' | 'in_progress' | 'completed') => { try { onMilestoneUpdated(await api.updateMilestoneStatus(milestone.id, status)); } catch (e) { setMilestoneError(e instanceof Error ? e.message : 'Unable to update milestone.'); } };
  return <><section className="page-intro"><div><p className="eyebrow">Project workspace</p><h1>Solutions in progress</h1><p>Follow approved proposals from active work through testing, deployment, and completion.</p></div></section><div className="two-column projects-layout"><section className="panel"><div className="panel-heading"><h2>Projects</h2><span>{projects.length} records</span></div>{projects.length === 0 ? <Empty text="No projects are available." /> : <div className="record-list">{projects.map((p) => <button className={`project-row ${selected?.id === p.id ? 'selected' : ''}`} key={p.id} onClick={() => setSelected(p)}><span className="record-code">PRJ-{p.id}</span><strong>{p.title}</strong><span>{label(p.stage)} · {label(p.approval_status)}</span></button>)}</div>}</section>{selected ? <section className="panel project-detail"><div className="panel-heading"><div><span className="record-code">PRJ-{selected.id}</span><h2>{selected.title}</h2></div><span className={`status status-${selected.stage}`}>{label(selected.stage)}</span></div><p>{selected.proposal_text}</p><h3>Milestones</h3><form className="inline-form" onSubmit={addMilestone}><input required minLength={3} value={milestoneTitle} onChange={(e) => setMilestoneTitle(e.target.value)} placeholder="Milestone title" /><input type="date" value={milestoneDueDate} onChange={(e) => setMilestoneDueDate(e.target.value)} /><button className="primary-button compact">Add milestone</button></form>{milestoneError && <div className="field-error">{milestoneError}</div>}<textarea rows={2} value={milestoneDescription} onChange={(e) => setMilestoneDescription(e.target.value)} placeholder="Optional milestone description" />{data.milestones.length ? data.milestones.map((m) => <div className="detail-line" key={m.id}><span><strong>{m.title}</strong><small>{m.description || ''}</small></span><span><select value={m.status} onChange={(e) => void updateMilestone(m, e.target.value as 'pending' | 'in_progress' | 'completed')}><option value="pending">Pending</option><option value="in_progress">In progress</option><option value="completed">Completed</option></select> · {dateText(m.due_date)}</span></div>) : <Empty text="No milestones recorded." />}<h3>Outcome</h3>{data.outcome ? <div className="metric-row"><b>{data.outcome.patents_filed} patents</b><b>{data.outcome.startups_created} startups</b><span>{data.outcome.impact_notes || 'Impact notes not recorded.'}</span></div> : <Empty text="No outcome recorded." />}<h3>Collaboration</h3><p>{data.partnerships.length} partnership requests · {data.messages.length} project messages</p></section> : <section className="panel"><Empty text="Select a project to view its milestones, outcomes, partnerships, and messages." /></section>}</div></>; }
