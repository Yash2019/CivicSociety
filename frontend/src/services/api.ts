...
export const api = {
  ...
  createMilestone: (id: number, data: Json) => request<Milestone>(`/projects/${id}/milestones`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) }),
  updateMilestoneStatus: (id: number, status: 'pending' | 'in_progress' | 'completed') => request<Milestone>(`/milestones/${id}/status`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status }) }),
